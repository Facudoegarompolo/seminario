...
const queueService = {
  ...
  llamarSiguiente: async (filaId = DEFAULT_FILA_ID) => {
    const response = await api.post(`/filas/${filaId}/llamar-siguiente`)
    return response.data
  },
  finalizarTurno: async (turnoId) => {
    const response = await api.post(`/turnos/${turnoId}/finalizar`)
    return response.data
  },
  marcarNoPresentado: async (turnoId) => {
    const response = await api.post(`/turnos/${turnoId}/no-presentado`)
    return response.data
  },
}
