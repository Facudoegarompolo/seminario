...
public class PublicTurnoController {
    ...
    @PostMapping("/{tokenPublico}/prioridad")
    public ResponseEntity<TurnoEstadoResponse> solicitarPrioridad(
            @PathVariable String tokenPublico) {
        TurnoEstadoResponse response = turnoService.solicitarPrioridad(tokenPublico);
        return ResponseEntity.ok(response);
    }
}
